      ...
      {!isConnected ? (
        ...
        <section className="flex h-[500px] w-[400px] flex-col gap-12">
          <div className="flex h-full w-full flex-col justify-between rounded-xl border-[1px] border-zinc-300 bg-zinc-50 p-8 dark:border-zinc-800 dark:bg-zinc-900 ">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex w-full">
                  <select
                    ...
                    onChange={(e) => setFromTokenSelection(options.find((o) => o.value === e.target.value) ?? options[0])}
                  >
                    {options.map((o, i) => (
                  ...
                  <input
                    ...
                    className="w-1/2 rounded-r-lg border-[1px] border-zinc-300 bg-zinc-100 p-4 text-lg font-bold text-zinc-950 placeholder:text-zinc-400 focus:outline-none dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-50 dark:placeholder:text-zinc-700"
                  />
                </div>
              </div>
              <div className="flex flex-col gap-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setToTokenSelection(options.find((o) => o.value === e.target.value) ?? options[3])
                  }
                >
                  {options.map((o, i) => (
                    <option key={i} value={o.value}>
            ...
            <button className=" w-full rounded-3xl bg-indigo-500 px-4 py-2 font-bold text-zinc-950 disabled:bg-zinc-300 disabled:text-zinc-500 dark:text-zinc-50 dark:disabled:bg-zinc-700" disabled={!amount}>
              Swap
            </button>
          </div>
        </section>
